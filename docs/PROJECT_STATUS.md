...
- Current production issue: None known; #50 is a visibility-only reduction pending review, with CEO-facing naming, one fast-loading top-level Management readout instead of duplicate KPIs (including minutes and operating cost), consistent first-load header chips without a Source label, bottom-positioned metric definitions, expanded sidebar navigation, polished Follow-up/Repeat engagement cards with normalized phone totals that match their profile drawers, repaired profile-drawer navigation, standardized operational CSV exports, compact failure reasons, a neutral Follow-up queue label, and an action-oriented Requested follow-ups view with visibly active readiness filters. The initial-render KPI-container regression has been fixed on the branch and requires deployment review.
